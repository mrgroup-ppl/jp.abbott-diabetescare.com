...
var s_account="abt-adc-test-inter";

if (whn.indexOf("abbottdiabetescare.com")>-1 || whn.indexOf("www.abbottdiabetescare.com")>-1){  // If production then send to production servers
    s_account="abt-adc-90081000-us-inter";
}
var s=s_gi(s_account);

/* Strip HTML tags */
var stripHtmlTags = function (title) {
...
var stripHtmlTagsFromArray = function(theArray){
    var i = 0;
    while(theArray[i] != null){
        theArray[i] = stripHtmlTags(theArray[i]);
        i++;
    }
    return theArray;
};

/* Grab file name for PDFs and non-HTML links */
var grabFileName = function (fileUrl) {
...
/* Conversion Config */
s.linkInternalFilters = "javascript:,myfreestyle.com,enter.myfreestyle.com";
...
s.formList="softwareRegistrationForm"; // List the forms which needs to be tracked//
...
s.siteid="jp-adc"            // leftmost value in pagename
...
function s_doPlugins(s) {
  ...
  s.prop50="ABT Enterprise v1.0" //code version
  if (!s.hier1) s.hier1 = s.pageName;
  /* Start of Page Naming */
  if(s.pageType=="errorPage"){
        s.pageName="";s.hier1="";s.channel="";}
  else if (s.pageName && s.hier1){
        var s_hps=s.split(s.hier1,"|");
        s_hps = stripHtmlTagsFromArray(s_hps);
        s.eVar24=s.prop24=s_hps[0];                                                                                             //country
        if(s_hps[1]){s.prop25=s.prop24+"|"+s_hps[1];}else{if(!s.hps){s.hps=s.prop24;}s.eVar25=s.prop25=s.hps;}         //site
        if(s_hps[2]){s.channel=s.prop25+"|"+s_hps[2];}else{if(!s.hps){s.hps=s.prop25;}s.eVar19=s.channel=s.hps;}       //section
        if(s_hps[3]){s.prop12=s.channel+"|"+s_hps[3];}else{if(!s.hps){s.hps=s.channel;}s.eVar20=s.prop12=s.hps;}       //sub section
        if(s_hps[4]){s.prop13=s.prop12+"|"+s_hps[4];}else{if(!s.hps){s.hps=s.prop12;}s.eVar21=s.prop13=s.hps;}         //sub section 2
        if(s_hps[5]){s.prop14=s.prop13+"|"+s_hps[5];}else{if(!s.hps){s.hps=s.prop13;}s.eVar22=s.prop14=s.hps;}         //sub section 3
  }
  ...
  // Hard coded site name as "My Free Style"//
  s.prop25 = "AbbottDiabetesCare";
  s.eVar25 = "D=c25";
  ...
  //if (!s.campaign) { s.campaign = s.getQueryParam('cid,responseTrackingCode', '|'); }
  if (!s.eVar61) { s.eVar61 = s.getQueryParam('responseTrackingCode'); s.prop61 = "D=v61";} // Tracking Response Tracking Code //
  if(s.campaign){
    s.campaign=s.getValOnce(s.campaign,"s_campaign",0);
    var cmp_ch = s.campaign;
    if(cmp_ch.indexOf("tv_")==0 || cmp_ch.indexOf("pr_")==0 || cmp_ch.indexOf("tr_")==0 || cmp_ch.indexOf("hcp_")==0 || cmp_ch.indexOf("Mc_")==0) {
      s.prop57 = "DTC";
      s.eVar57 = "D=c57";
      s_setPCk("dtc_visit","1");
    }
    else {
      s.prop57 = "Non DTC";
      s.eVar57 = "D=c57";
      s_setPCk("dtc_visit","0");      
    }
  }
  var tmp_ck_val = s_rCk("dtc_visit");
  if (tmp_ck_val == "1"){
    s.prop57 = "DTC";
    s.eVar57 = "D=c57";
  }
  ...
  if (s.events && s.events.indexOf("event2")!=-1) {
    s_setPCk("enroll_status","1");
    s.prop58 = "Post Enrollment";
    s.eVar58 = "D=c58";
  }else {
    s.prop58 = "Pre Enrollment";
    s.eVar58 = "D=c58";
  }
  tmp_ck_val1 = s_rCk("enroll_status");
  if (tmp_ck_val1 == "1"){
    s.prop58 = "Post Enrollment";
    s.eVar58 = "D=c58";
  }
  ...
  // Abbott Vs Non Abbott Settings //
  if (s.prop60) {
    s.eVar60 = "D=c60";
  }
  
  /*********************************************************************
  * Start of Internal Search
    *********************************************************************/

    /* Internal Search Term */
    if(s.getQueryParam('queryStr')){
            s.prop16 = s.getQueryParam('queryStr').toLowerCase();          //search term
            s.events=s.apl(s.events,'event5',',',1);
    }
    if(s.prop16) {//search term
        s.events=s.apl(s.events,'event5',',',1);
    }
    if(s.prop16){s.eVar16="D=c16";}
       
  /* Search Page Origination */
  s.tempValue=s.getPreviousValue(s.pageName,'gpv_p17')
  if(s.events){
    if(s.events.indexOf('event5')>-1){
      s.prop17 = s.tempValue;
      s.eVar17 = "D=c17";
        }
    }

    /* search Result Type */
  if(s.prop41) {s.eVar41=s.prop41=s.prop41.toLowerCase()}         //search result type

    /* Click Throughs --------------------------------*/

    /* Click Through Link */
  if(s.eVar27){
        s.events = ""; // clear s.events so that event5 is not still reported
        s.events=s.apl(s.events,'event6',',',1) ;
    }/* ---------------------------------------------- */

    /* Search Results Page Name */
    if(s.events=='event5'){
        if(s.pageName.indexOf('|SearchResults'==-1)){s.pageName=s.pageName+"|SearchResults";}
    }
  ...
  * End of Internal Search
  ...
  if(s.pageName && s.prop30) s.prop31=s.prop30+':'+s.pageName;    //user type pathing
  s.prop48=s.getNewRepeat(30);                           //New vs. Repeat
  s.eVar48 = "D=c48";
  
  /* Internal Promotion */
  if(!s.eVar44){s.eVar44=s.getQueryParam('icid');}
  if(s.eVar44){
  var int_cmp=s.getValOnce(s.eVar44,'s_ev44',0);
  if(int_cmp) s.events=s.apl(s.events,'event8',',',0);}

  /* Custom File Download and Exit Tracking */
  s.linkLeaveQueryString=true;
  var eurl=s.exitLinkHandler()
  var durl=s.linkHandler('.exe|.zip|.wav|.mp3|.mov|.mpg|.avi|.wmv|.pdf|.doc|.docx|.xls|.xlsx|.ppt|.pptx','d')
  if(durl){
      s.prop36_original=s.prop36; //download link
            s.eVar36_original=s.eVar36; //download link
      s.prop38_original=s.prop38; //download page
            s.ltv_original=s.linkTrackVars;s.lte_original=s.linkTrackEvents;s.events_original=s.events;
            s.linkTrackVars="prop36,prop38,eVar36,events";s.linkTrackEvents="event11";
      s.events="event11";s.prop36=s.eVar36=durl;s.prop38=s.pageName?s.pageName:s.wd.location;
        }
        else if(eurl){
      s.prop37_original=s.prop37; //exit link
            s.eVar37_original=s.eVar37; //exit link
      s.prop38_original=s.prop38; //exit page
            s.ltv_original=s.linkTrackVars;s.lte_original=s.linkTrackEvents;s.events_original=s.events;
      s.linkTrackVars="prop37,prop38,eVar37,events";s.linkTrackEvents="event12";
      s.events="event12";s.prop37=s.eVar37=eurl;s.prop38=s.pageName?s.pageName:s.wd.location;
        }
        else{// restore pre-linkHandler variable values
            if(s.ltv_original)s.linkTrackVars=s.ltv_original;if(s.ltv_original)s.linkTrackEvents=s.lte_original;
            if(s.prop36_original)s.prop36=s.prop36_original;
            if(s.prop37_original)s.prop37=s.prop37_original;
      if(s.prop38_original)s.prop38=s.prop38_original;
            if(s.eVar36_original)s.eVar36=s.eVar36_original;
      if(s.eVar37_original)s.eVar37=s.eVar37_original;
            if(s.events_original)s.events=s.events_original;
        }
        s.linkLeaveQueryString=false;
    
    /* Form Analysis Function Call */
       s.setupFormAnalysis(); //Form Abandonment

    /* Link Tracking */
    //s.hbx_lt="auto";
    //s.setupLinkTrack("prop31,prop32,prop15,prop33","SiteC_LINKS");
    ...
    s.prop46=s.getTimeParting('h','-6',theYear); // Set hour
    s.prop47=s.getTimeParting('d','-6',theYear); // Set day
    
    s.eVar46 = "D=c46";
    s.eVar47 = "D=c47";
    
  /* Article Finding Method */
  if(s.eVar16){s.c_w("afm","keywordSearch",0);s.eVar38="Internal Search";}
  if(s.eVar44){s.c_w("afm","intPromo",0);s.eVar38="Internal Promotion";}
  if(s.events){
      if(s.events.indexOf('event9')>-1){
        if(s.c_r("afm")){(s.c_w("afm","",0));}
        else{s.eVar38="Browse"}
      }
  }

  /* Copy props to eVars */
  if(s.prop15){s.eVar15="D=c15";} //language
  if(s.prop24) {s.eVar24="D=c24";}//country
  if(s.prop30){ //user type
    s.eVar30="D=c30";
    s_setSCk("ck_loggedin","1");
  } 
  tmp_isloggedin = s_rCk("ck_loggedin");
  if (tmp_isloggedin == "1") {
    s.prop30 = "Logged In";
    s.eVar30 = "D=c30";
  }else {
    s.prop30 = "Not Logged In";
    s.eVar30="D=c30";
  }
}
...
s.visitorNamespace="abbottlabs";
s.trackingServer="metrics.abbott.com";
s.trackingServerSecure="smetrics.abbott.com";
//s.trackingServer="abbottlabs.112.2o7.net";
...
function s_setSCk(k,v){
/*
var d=".abbott.com";if(k)document.cookie=k+
'='+escape(v)+'; path=/;'+(d?' domain='+d+';':'');
*/
  s.c_w(k,v);
}
function s_setPCk(k,v){s_setPCkE(k,v,1825);}
function s_setPCkE(k,v,dt){
    var exp=new Date;
    //var d=".abbott.com";
...
/*
 * Plugin: setupLinkTrack 2.05b - return links for link tracking in
 * SiteCatalyst 
 */
